...
class AdminSettingsController extends Controller
{
    ...
    {
        $request->validate([
            'global_logo' => 'nullable|image|mimes:jpeg,jpg,png,gif,webp,svg|max:5120',
        ]);
    ...
    public function updateHero(Request $request)
    {
        $request->validate([
            ...
            'hero_gradient_color' => 'nullable|string|max:7',
            'hero_text_color' => 'nullable|string|max:7',
            'hero_title' => 'nullable|string|max:500',
            'hero_subtitle' => 'nullable|string|max:1000',
            'hero_cta_text' => 'nullable|string|max:100',
            'hero_background_image' => 'nullable|image|mimes:jpeg,jpg,png,gif,webp|max:5120',
            'remove_hero_background_image' => 'nullable|boolean',
        ]);

        $settings = $this->getCurrentSettings();

        $settings['hero'] = array_merge($settings['hero'] ?? [], [
            'background_color' => $request->hero_background_color ?? $settings['hero']['background_color'] ?? '#667eea',
            'gradient_color' => $request->hero_gradient_color ?? $settings['hero']['gradient_color'] ?? '#764ba2',
            'text_color' => $request->hero_text_color ?? $settings['hero']['text_color'] ?? '#ffffff',
            'title' => $request->hero_title ?? $settings['hero']['title'] ?? 'Review Smarter. Learn Better. Succeed Faster.',
            'subtitle' => $request->hero_subtitle ?? $settings['hero']['subtitle'] ?? 'At Ascendo Review and Training Center, we guide future licensed professionals toward exam success with expert-led reviews and flexible learning options.',
            'cta_text' => $request->hero_cta_text ?? $settings['hero']['cta_text'] ?? 'ENROLL NOW',
        ]);

        if ($request->input('remove_hero_background_image')) {
            if (isset($settings['hero']['background_image']) && Storage::disk('public')->exists($settings['hero']['background_image'])) {
                Storage::disk('public')->delete($settings['hero']['background_image']);
            }
            unset($settings['hero']['background_image']);
        }
        ...
        if ($request->hasFile('hero_background_image')) {
            ...
            $imagePath = $request->file('hero_background_image')->store('settings/hero', 'public');
            $settings['hero']['background_image'] = $imagePath;
        }
    ...
    public function updateProgramsSection(Request $request)
    {
        $request->validate([
            'programs_background_color' => 'nullable|string|max:7',
            'programs_gradient_color' => 'nullable|string|max:7',
            'programs_text_color' => 'nullable|string|max:7',
            'programs_title' => 'nullable|string|max:255',
            'programs_subtitle' => 'nullable|string|max:500',
            'programs_background_image' => 'nullable|image|mimes:jpeg,jpg,png,gif,webp|max:5120',
            'remove_programs_background_image' => 'nullable|boolean',
        ]);

        $settings = $this->getCurrentSettings();

        $settings['programs_section'] = array_merge($settings['programs_section'] ?? [], [
            'background_color' => $request->programs_background_color ?? $settings['programs_section']['background_color'] ?? '#f8f9fa',
            'gradient_color' => $request->programs_gradient_color ?? $settings['programs_section']['gradient_color'] ?? '',
            'text_color' => $request->programs_text_color ?? $settings['programs_section']['text_color'] ?? '#333333',
            'title' => $request->programs_title ?? $settings['programs_section']['title'] ?? 'Programs Offered',
            'subtitle' => $request->programs_subtitle ?? $settings['programs_section']['subtitle'] ?? 'Choose from our comprehensive review programs designed for success',
        ]);

        if ($request->input('remove_programs_background_image')) {
            if (isset($settings['programs_section']['background_image']) && Storage::disk('public')->exists($settings['programs_section']['background_image'])) {
                Storage::disk('public')->delete($settings['programs_section']['background_image']);
            }
            unset($settings['programs_section']['background_image']);
        }

        if ($request->hasFile('programs_background_image')) {
            if (isset($settings['programs_section']['background_image']) && Storage::disk('public')->exists($settings['programs_section']['background_image'])) {
                Storage::disk('public')->delete($settings['programs_section']['background_image']);
            }
            $imagePath = $request->file('programs_background_image')->store('settings/programs', 'public');
            $settings['programs_section']['background_image'] = $imagePath;
        }

        $this->saveSettings($settings);

        return back()->with('success', 'Programs section settings updated successfully!');
    }

    public function updateModalitiesSection(Request $request)
    {
        $request->validate([
            'modalities_background_color' => 'nullable|string|max:7',
            'modalities_gradient_color' => 'nullable|string|max:7',
            ...
            'modalities_background_image' => 'nullable|image|mimes:jpeg,jpg,png,gif,webp|max:5120',
            'remove_modalities_background_image' => 'nullable|boolean',
        ]);

        $settings = $this->getCurrentSettings();

        $settings['modalities_section'] = array_merge($settings['modalities_section'] ?? [], [
            'background_color' => $request->modalities_background_color ?? $settings['modalities_section']['background_color'] ?? '#667eea',
            'gradient_color' => $request->modalities_gradient_color ?? $settings['modalities_section']['gradient_color'] ?? '#764ba2',
            'text_color' => $request->modalities_text_color ?? $settings['modalities_section']['text_color'] ?? '#ffffff',
            'title' => $request->modalities_title ?? $settings['modalities_section']['title'] ?? 'Available Modalities',
            'subtitle' => $request->modalities_subtitle ?? $settings['modalities_section']['subtitle'] ?? 'Choose the learning style that fits your schedule and preferences',
        ]);

        if ($request->input('remove_modalities_background_image')) {
            if (isset($settings['modalities_section']['background_image']) && Storage::disk('public')->exists($settings['modalities_section']['background_image'])) {
                Storage::disk('public')->delete($settings['modalities_section']['background_image']);
            }
            unset($settings['modalities_section']['background_image']);
        }

        if ($request->hasFile('modalities_background_image')) {
            if (isset($settings['modalities_section']['background_image']) && Storage::disk('public')->exists($settings['modalities_section']['background_image'])) {
                Storage::disk('public')->delete($settings['modalities_section']['background_image']);
            }
            $imagePath = $request->file('modalities_background_image')->store('settings/modalities', 'public');
            $settings['modalities_section']['background_image'] = $imagePath;
        }

        $this->saveSettings($settings);

        return back()->with('success', 'Modalities section settings updated successfully!');
    }

    public function updateAboutSection(Request $request)
    {
        $request->validate([
            'about_background_color' => 'nullable|string|max:7',
            'about_gradient_color' => 'nullable|string|max:7',
            'about_text_color' => 'nullable|string|max:7',
            'about_title' => 'nullable|string|max:255',
            'about_content' => 'nullable|string|max:2000',
            'about_background_image' => 'nullable|image|mimes:jpeg,jpg,png,gif,webp|max:5120',
            'remove_about_background_image' => 'nullable|boolean',
        ]);

        $settings = $this->getCurrentSettings();

        $settings['about_section'] = array_merge($settings['about_section'] ?? [], [
            'background_color' => $request->about_background_color ?? $settings['about_section']['background_color'] ?? '#ffffff',
            'gradient_color' => $request->about_gradient_color ?? $settings['about_section']['gradient_color'] ?? '',
            'text_color' => $request->about_text_color ?? $settings['about_section']['text_color'] ?? '#333333',
            'title' => $request->about_title ?? $settings['about_section']['title'] ?? 'ABOUT US',
            'content' => $request->about_content ?? $settings['about_section']['content'] ?? 'Ascendo Review and Training Center is dedicated to providing exceptional review programs and training services for professional licensure examinations. With experienced instructors, comprehensive materials, and proven methodologies, we help aspiring professionals achieve their career goals through excellence in education and training.',
        ]);

        if ($request->input('remove_about_background_image')) {
            if (isset($settings['about_section']['background_image']) && Storage::disk('public')->exists($settings['about_section']['background_image'])) {
                Storage::disk('public')->delete($settings['about_section']['background_image']);
            }
            unset($settings['about_section']['background_image']);
        }
        ...
        if ($request->hasFile('about_background_image')) {
            ...
            $imagePath = $request->file('about_background_image')->store('settings/about', 'public');
            $settings['about_section']['background_image'] = $imagePath;
        }

        $this->saveSettings($settings);

        return back()->with('success', 'About section settings updated successfully!');
    }

    public function updateHomepageGeneral(Request $request)
    {
        $request->validate([
            'homepage_background_color' => 'nullable|string|max:7',
            'homepage_gradient_color' => 'nullable|string|max:7',
            'homepage_text_color' => 'nullable|string|max:7',
            'homepage_background_image' => 'nullable|image|mimes:jpeg,jpg,png,gif,webp|max:5120',
        ...
        $settings['homepage'] = array_merge($settings['homepage'] ?? [], [
            'background_color' => $request->homepage_background_color ?? $settings['homepage']['background_color'] ?? '#f8f9fa',
            'gradient_color' => $request->homepage_gradient_color ?? $settings['homepage']['gradient_color'] ?? '',
            'text_color' => $request->homepage_text_color ?? $settings['homepage']['text_color'] ?? '#333333',
        ]);

        if ($request->input('remove_homepage_background_image')) {
            if (isset($settings['homepage']['background_image']) && Storage::disk('public')->exists($settings['homepage']['background_image'])) {
                Storage::disk('public')->delete($settings['homepage']['background_image']);
            }
            unset($settings['homepage']['background_image']);
        ...
        return back()->with('success', 'Homepage general settings updated successfully!');
    }
