...
<link rel="stylesheet" href="{{ asset('css/admin/admin-modules/admin-modules.css') }}?v={{ time() }}">
<style>
...
<div class="edit-module-container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1>Edit Module: {{ $module->module_name }}</h1>
        <a href="{{ route('admin.modules.index', ['program_id' => $module->program_id]) }}" class="btn btn-secondary">
            ← Back to Modules
        </a>
    </div>
    ...
    <!-- Main Module Form -->
    <form action="{{ route('admin.modules.update', $module->modules_id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <textarea id="module_description" name="module_description" class="form-control" rows="4">{{ old('module_description', $module->module_description) }}</textarea>
        </div>
            ...
            <select id="program_id" name="program_id" class="form-control" required>
                ...
                @foreach($programs as $program)
                    <option value="{{ $program->program_id }}" 
                            {{ old('program_id', $module->program_id) == $program->program_id ? 'selected' : '' }}>
        ...
        @if($module->attachment)
            <div class="current-attachment">
                ...
                <p>{{ basename($module->attachment) }}</p>
                <a href="{{ asset('storage/' . $module->attachment) }}" target="_blank" class="btn btn-sm btn-outline-primary">
                    View Current File
                </a>
            </div>
        @endif
    ...
    <div class="content-section">
        ...
        <div class="additional-content" id="additionalContent">
            @if($module->additional_content)
                ...
                @endphp
                @foreach($additionalContent as $index => $content)
                    <div class="content-item">
                        <h6>{{ $content['title'] ?? 'Untitled Content' }}</h6>
                        <p class="text-muted">{{ $content['description'] ?? 'No description' }}</p>
                        <span class="badge badge-info">{{ ucfirst($content['type'] ?? 'unknown') }}</span>
                        @if(isset($content['file_path']))
                            <a href="{{ asset('storage/' . $content['file_path']) }}" target="_blank" class="btn btn-sm btn-outline-primary ml-2">
                                View File
                            </a>
                        @elseif(isset($content['url']))
                            <a href="{{ $content['url'] }}" target="_blank" class="btn btn-sm btn-outline-primary ml-2">
                                Visit Link
                            </a>
                        @endif
                    </div>
...
function uploadVideo(file) {
    ...
    fetch('{{ route("admin.modules.upload-video", $module->modules_id) }}', {
        method: 'POST',
        headers: {
...
function addContent() {
    ...
    fetch('{{ route("admin.modules.add-content", $module->modules_id) }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        },
