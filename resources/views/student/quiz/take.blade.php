...
<head>
    ...
    <title>{{ $quiz->quiz_title }} - Quiz - A.R.T.C</title>
...
<div class="quiz-container">
    ...
    <div class="quiz-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="mb-0 opacity-75">{{ $quiz->questions->count() }} questions</p>
            </div>
            <div class="text-end">
                <div class="student-info position-relative">
                    <strong>{{ $student->student_fname ?? $student->first_name ?? 'Student' }} {{ $student->student_lname ?? $student->last_name ?? '' }}</strong>
                    <br>
                    <small class="opacity-75">{{ $student->student_id ?? $student->student_number ?? 'N/A' }}</small>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="questions-panel">
            <form id="quizForm">
                ...
                @foreach($questions as $index => $question)
                    <div class="question-card" id="question-{{ (int)$index + 1 }}" style="{{ (int)$index > 0 ? 'display: none;' : '' }}">
                                ...
                                @foreach($options as $optionIndex => $option)
                                    <div class="answer-option">
                                        <input type="radio" 
                                               ...
                                               value="{{ chr(65 + (int)$optionIndex) }}" 
                                               id="q{{ $question->id }}_{{ chr(65 + (int)$optionIndex) }}"
                                               onchange="markAnswered({{ (int)$index + 1 }})">
                                        <label for="q{{ $question->id }}_{{ chr(65 + (int)$optionIndex) }}">
                                            <strong>{{ chr(65 + (int)$optionIndex) }}.</strong> {{ $option }}
                                        </label>
                                    </div>
                                @endforeach
                            @endif
        ...
        <div class="sidebar-panel">
            ...
            @if($timeRemaining !== null)
                <div class="timer-display" id="timerDisplay">
                    ...
                    <div class="timer-value" id="timerValue">{{ $timeRemaining }}:00</div>
                </div>
            @endif
            ...
            <div class="progress-overview">
                ...
                <div class="d-flex justify-content-between mb-2">
                    <span>Remaining:</span>
                    <span id="remainingCount">{{ $quiz->questions->count() }}</span>
                </div>
                ...
                <div class="question-grid">
                    @foreach($questions as $index => $question)
                        <button type="button" 
                                class="question-btn {{ (int)$index === 0 ? 'current' : '' }}" 
                                id="qBtn-{{ (int)$index + 1 }}"
                                onclick="goToQuestion({{ (int)$index + 1 }})">
                            {{ (int)$index + 1 }}
                        </button>
                    @endforeach
                </div>
    ...
    <div class="quiz-actions">
        ...
        <div class="quiz-info">
            <span class="text-muted fw-bold">Question <span id="currentQuestion">1</span> of {{ $quiz->questions->count() }}</span>
        </div>
    </div>
</div>
...
<script>
    ...
    const timeLimit = {{ $timeRemaining ?? 0 }};
