{{-- Real-time Chat Component with Laravel Echo --}}
@push('scripts')
<script>
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentUserId = @json(auth()->check() ? auth()->user()->user_id : (session('professor_id') ?: session('director_id')));
    const userRole = @json(auth()->check() ? auth()->user()->role : (session('professor_id') ? 'professor' : 'director'));
