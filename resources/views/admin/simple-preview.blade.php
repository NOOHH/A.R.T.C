...
<body>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-3">
                <div class="stat-card">
                    <div class="stat-number">{{ $analytics['total_students'] ?? 0 }}</div>
                    <div class="text-muted">Total Students</div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="stat-card">
                    <div class="stat-number">{{ $analytics['total_programs'] ?? 0 }}</div>
                    <div class="text-muted">Total Programs</div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="stat-card">
                    <div class="stat-number">{{ $analytics['total_enrollments'] ?? 0 }}</div>
                    <div class="text-muted">Total Enrollments</div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="stat-card">
                    <div class="stat-number">{{ $analytics['pending_registrations'] ?? 0 }}</div>
                                ...
                                <table class="table">
                                    ...
                                    <tbody>
                                        @foreach($registrations as $reg)
                                            <tr>
                                                <td>{{ $reg->user_firstname ?? 'N/A' }} {{ $reg->user_lastname ?? '' }}</td>
                                                <td>{{ $reg->user_email ?? 'N/A' }}</td>
                                                <td>{{ $reg->program_name ?? 'N/A' }}</td>
                                                <td>
                                                    <span class="badge bg-warning">{{ ucfirst($reg->status ?? 'pending') }}</span>
                                                </td>
                                                <td>${{ number_format($reg->total_amount ?? 0, 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
