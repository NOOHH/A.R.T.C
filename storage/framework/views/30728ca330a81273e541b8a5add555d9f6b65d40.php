...
<?php $__env->startSection('title', 'Payment Rejected'); ?>
        ...
        <div class="col-12">
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex justify-content-between align-items-center bg-danger text-white">
                    ...
                    <div>
                        <small>Total: <?php echo e($payments->count()); ?></small>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" width="100%" cellspacing="0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <?php if(isset($payment->registration)): ?>
                                                <?php echo e(($payment->registration->firstname ?? '')); ?> 
                                                <?php echo e(($payment->registration->middlename ?? '')); ?>

                                                <?php echo e(($payment->registration->lastname ?? '')); ?>
                                        ...
                                        <td>
                                            <?php if(isset($payment->registration->user) && $payment->registration->user): ?>
                                                <?php echo e($payment->registration->user->email ?? 'N/A'); ?>
                                        ...
                                        <td>
                                            <?php if(isset($payment->registration)): ?>
                                                <?php echo e($payment->registration->program_name ?? 'N/A'); ?>
                                        ...
                                        <td><?php echo e($payment->rejected_at ? \Carbon\Carbon::parse($payment->rejected_at)->format('M d, Y g:i A') : 'N/A'); ?></td>
                                        <td>
                                            <?php if($payment->rejected_fields): ?>
                                                <?php
                                                ...
                                                <?php $__currentLoopData = $rejectedFields; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <span class="rejected-field badge badge-danger"><?php echo e($field); ?></span>
                                        ...
                                        <td>
                                            <span class="text-truncate" style="max-width: 150px; display: inline-block;" title="<?php echo e($payment->rejection_reason); ?>">
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                <button class="btn btn-info btn-sm" onclick="viewRejectedPayment(<?php echo e($payment->payment_id); ?>)">
                                                    <i class="bi bi-eye"></i> View
                                                </button>
                                                <button class="btn btn-warning btn-sm" onclick="editPaymentRejection(<?php echo e($payment->payment_id); ?>)">
                                                ...
                                                <button class="btn btn-success btn-sm" onclick="approvePayment(<?php echo e($payment->payment_id); ?>)">
                        ...
                        <table class="table table-bordered" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>₱<?php echo e(number_format($payment->amount, 2)); ?></td>
                                    <td><?php echo e($payment->payment_method ?? 'N/A'); ?></td>
                                    <td><?php echo e($payment->resubmitted_at ? \Carbon\Carbon::parse($payment->resubmitted_at)->format('M d, Y g:i A') : 'N/A'); ?></td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            <button class="btn btn-primary btn-sm" onclick="viewPaymentComparison(<?php echo e($payment->payment_id); ?>)">
                                                <i class="bi bi-layout-split"></i> Compare
                                            </button>
                                            <button class="btn btn-success btn-sm" onclick="approvePaymentResubmission(<?php echo e($payment->payment_id); ?>)">
                                                <i class="bi bi-check"></i> Approve
                                            </button>
                                            <button class="btn btn-danger btn-sm" onclick="rejectPaymentAgain(<?php echo e($payment->payment_id); ?>)">
...
function viewRejectedPayment(paymentId) {
    ...
    fetch(`<?php echo e(route('admin.payments.details', '')); ?>/${paymentId}`)
...
function viewPaymentComparison(paymentId) {
    ...
    Promise.all([
        fetch(`<?php echo e(route('admin.payments.details', '')); ?>/${paymentId}`),
        fetch(`<?php echo e(route('admin.payments.original-data', '')); ?>/${paymentId}`)
    ])
...
function approvePayment(paymentId) {
    if (confirm('Are you sure you want to approve this payment?')) {
        fetch(`<?php echo e(route('admin.payments.approve', '')); ?>/${paymentId}`, {
            method: 'POST',
            headers: {
...
function approvePaymentResubmission(paymentId) {
    if (confirm('Are you sure you want to approve this payment resubmission?')) {
        fetch(`<?php echo e(route('admin.payments.approve-resubmission', '')); ?>/${paymentId}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
document.getElementById('editPaymentRejectionForm').addEventListener('submit', function(e) {
    ...
    fetch(`<?php echo e(route('admin.payments.update-rejection', '')); ?>/${paymentId}`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        },
...
<?php echo $__env->make('admin.admin-dashboard.admin-dashboard-layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\A.R.T.C\resources\views\admin\admin-student-registration\admin-payment-rejected.blade.php ENDPATH**/ ?>