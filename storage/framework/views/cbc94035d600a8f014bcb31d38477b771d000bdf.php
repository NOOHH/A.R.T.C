            ...
            <ul class="nav nav-tabs card-header-tabs" id="quizTabs" role="tablist">
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="draft-tab" data-bs-toggle="tab" data-bs-target="#draft" type="button" role="tab">
                        <i class="bi bi-file-text"></i> Draft <span class="badge bg-warning ms-1"><?php echo e($draftQuizzes->count()); ?></span>
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="published-tab" data-bs-toggle="tab" data-bs-target="#published" type="button" role="tab">
                        <i class="bi bi-check-circle"></i> Published <span class="badge bg-success ms-1"><?php echo e($publishedQuizzes->count()); ?></span>
                    </button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="archived-tab" data-bs-toggle="tab" data-bs-target="#archived" type="button" role="tab">
                        <i class="bi bi-archive"></i> Archived <span class="badge bg-secondary ms-1"><?php echo e($archivedQuizzes->count()); ?></span>
                    </button>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="quizTabsContent">
                ...
                <div class="tab-pane fade show active" id="draft" role="tabpanel">
                    ...
                    <?php else: ?>
                        <?php echo $__env->make('admin.quiz-generator.quiz-table', ['quizzes' => $draftQuizzes, 'status' => 'draft'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    <?php endif; ?>
                </div>

                <!-- Published Quizzes -->
                <div class="tab-pane fade" id="published" role="tabpanel">
                    <?php if($publishedQuizzes->isEmpty()): ?>
                        <div class="text-center py-4">
                    ...
                    <?php else: ?>
                        <?php echo $__env->make('admin.quiz-generator.quiz-table', ['quizzes' => $publishedQuizzes, 'status' => 'published'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    <?php endif; ?>
                </div>

                <!-- Archived Quizzes -->
                <div class="tab-pane fade" id="archived" role="tabpanel">
                    <?php if($archivedQuizzes->isEmpty()): ?>
                        <div class="text-center py-4">
                    ...
                    <?php else: ?>
                        <?php echo $__env->make('admin.quiz-generator.quiz-table', ['quizzes' => $archivedQuizzes, 'status' => 'archived'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                    <?php endif; ?>
                </div>
            </div>
        </div>
...
console.log('DEBUG Auth::user()', <?php echo json_encode(Auth::user(), 15, 512) ?>);
console.log('DEBUG Auth::guard("director")->user()', <?php echo json_encode(Auth::guard('director')->user(), 15, 512) ?>);

// Test CSRF token
console.log('DEBUG CSRF Token:', document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'));

// Test programs data
console.log('DEBUG Programs:', <?php echo json_encode($assignedPrograms, 15, 512) ?>);

// Global variables - Updated for admin context
window.adminQuizGenerator = {
    csrfToken: document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
    baseUrl: '<?php echo e(url("/")); ?>',
    programs: <?php echo json_encode($assignedPrograms, 15, 512) ?>,
    currentUser: <?php echo json_encode(Auth::user(), 15, 512) ?>,
    sessionData: <?php echo json_encode(session()->all(), 15, 512) ?>,
    userRole: 'admin',
    myId: <?php echo json_encode(session('user_id'), 15, 512) ?>,
    myName: '<?php echo json_encode(session('user_name'), 15, 512) ?>',
    isAuthenticated: <?php echo json_encode(session('logged_in'), 15, 512) ?>
};
...
<?php echo $__env->make('admin.admin-dashboard.admin-dashboard-layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\A.R.T.C\resources\views\admin\quiz-generator\index.blade.php ENDPATH**/ ?>