...
<body class="login-page">
    ...
    <div class="right">
        ...
        <form class="login-form" method="POST" action="<?php echo e(route('user.signup')); ?>" id="signupForm">
            <?php echo csrf_field(); ?>
                         <div class="row mb-3">
                 <div class="col-md-6">
                     <label for="first_name">First Name</label>
                     <input type="text" id="first_name" name="first_name" placeholder="First Name" value="<?php echo e(old('first_name')); ?>" required>
                 </div>
                 <div class="col-md-6">
                     <label for="last_name">Last Name</label>
                     <input type="text" id="last_name" name="last_name" placeholder="Last Name" value="<?php echo e(old('last_name')); ?>" required>
                 </div>
             </div>
            ...
            <label for="referral_code">
                Referral Code 
                <?php if(DB::table('admin_settings')->where('setting_key', 'referral_required')->value('setting_value') === '1'): ?>
            ...
            <!-- reCAPTCHA -->
            <div class="g-recaptcha" data-sitekey="<?php echo e(env('RECAPTCHA_SITE_KEY')); ?>" style="margin: 20px 0;"></div>
            ...
            <div style="margin-top: 8px; font-size: 1em;">Already have an account? <a href="<?php echo e(route('login')); ?>" class="register-link">Login here.</a></div>
        </form>
    </div>
    <script>
        ...
        async function sendOTP() {
            ...
            try {
                const response = await fetch('<?php echo e(route("signup.send.otp")); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
        ...
        async function verifyOTP() {
            ...
            try {
                const response = await fetch('<?php echo e(route("signup.verify.otp")); ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
<?php /**PATH C:\xampp\htdocs\A.R.T.C\resources\views\Login\signup.blade.php ENDPATH**/ ?>