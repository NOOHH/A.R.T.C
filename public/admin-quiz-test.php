<?php
// Create a route for testing admin quiz functionality
...
$request = Illuminate\Http\Request::create('/admin-quiz-test', 'GET');
...
<body>
    <div class="container mt-4">
        ...
        <div class="alert alert-info">
            ...
            <p><strong>Admin ID:</strong> <?php echo session('user_id', 'Not set'); ?></p>
            <p><strong>CSRF Token:</strong> <?php echo substr($csrfToken, 0, 20); ?>...</p>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mt-3">
                    <button class="btn btn-info btn-sm" onclick="checkQuizStatus()">
                        <i class="bi bi-refresh"></i> Check Current Quiz Status
                    </button>
    ...
    <script>
        ...
        async function checkQuizStatus() {
            log('Checking current quiz status...');
            
            try {
                // Check quiz 46 (admin quiz)
                const response46 = await fetch('/api/quiz/46/status', {
                    method: 'GET',
                ...
                if (response46.ok) {
                    ...
                    log(`Quiz 46 (Admin): ${JSON.stringify(data46)}`);
                } else {
                    log(`Quiz 46 status check failed: ${response46.status}`);
                }
                
                // Check quiz 48 (professor quiz)
                const response48 = await fetch('/api/quiz/48/status', {
                    method: 'GET',
                    credentials: 'same-origin'
                });
                
                if (response48.ok) {
                    const data48 = await response48.json();
                    log(`Quiz 48 (Professor): ${JSON.stringify(data48)}`);
                } else {
                    log(`Quiz 48 status check failed: ${response48.status}`);
                }
