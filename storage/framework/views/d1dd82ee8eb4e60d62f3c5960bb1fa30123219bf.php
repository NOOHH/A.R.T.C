...
<body class="login-page">
    <?php
        ...
        // Try to get tenant-specific settings if available
        $tenantSlug = $tenantSlug ?? (request()->attributes->get('tenant_slug') ?? null);
        if ($tenantSlug) {
            $tenant = \App\Models\Tenant::where('slug', $tenantSlug)->first();
            if ($tenant) {
                $tenantSettings = \App\Helpers\TenantContextHelper::loadTenantSettings($tenant);
                $auth = $tenantSettings['auth'] ?? $auth;
