<script type="text/javascript">
    ...
    async function handleFormSubmission(event, settingType, loadingText) {
        ...
        try {
            ...
            switch(settingType) {
                case 'general':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.general", ["website" => $selectedWebsite->id])); ?>';
                    break;
                case 'branding':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.branding", ["website" => $selectedWebsite->id])); ?>';
                    break;
                case 'navbar':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.navbar", ["website" => $selectedWebsite->id])); ?>';
                    break;
                case 'homepage':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.homepage", ["website" => $selectedWebsite->id])); ?>';
                    break;
                case 'student':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.student", ["website" => $selectedWebsite->id])); ?>';
                    break;
                case 'professor':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.professor", ["website" => $selectedWebsite->id])); ?>';
                    break;
                case 'admin':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.admin", ["website" => $selectedWebsite->id])); ?>';
                    break;
                case 'advanced':
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.advanced", ["website" => $selectedWebsite->id])); ?>';
                    break;
                default:
                    endpoint = '<?php echo e(route("smartprep.dashboard.settings.update.general", ["website" => $selectedWebsite->id])); ?>';
            }
    ...
    function updatePreviewForSection(section) {
        ...
        let previewUrl = '<?php echo e($previewUrl); ?>';
        ...
        if (iframe && loading) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Add website parameter to ensure tenant-specific settings are loaded
    ...
    async function initializePreviewUrl() {
        try {
            ...
            if (response.ok) {
                const settings = await response.json();
                const fallbackUrlInit = "<?php echo e($previewUrl); ?>";
    ...
    function saveSidebarColorsForRole(role, colors) {
        fetch('<?php echo e(route("smartprep.dashboard.settings.update.sidebar", ["website" => $selectedWebsite->id])); ?>', {
            method: 'POST',
            headers: {
