<script type="text/javascript">
    ...
    // Initialize preview URL from settings (after restoring any stored panel state)
    ...
    try {
        ...
        localStorage.setItem(lsBaseKey, previewUrl);
        localStorage.setItem(lsSectionKey, section);
    } catch(e){}
        ...
        if (iframe && loading) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // Restore stored base if any
                    let iframeBase = (iframe && (iframe.dataset.previewBase || iframe.getAttribute('data-preview-base')) ) ? (iframe.dataset.previewBase || iframe.getAttribute('data-preview-base')) : null;
                    if(!iframeBase) {
                        try {
                            const lsBaseKey = 'preview_base_customize_' + websiteId;
                            iframeBase = localStorage.getItem(lsBaseKey) || null;
                        } catch(e){}
                    }
    ...
    async function initializePreviewUrl() {
        try {
            ...
            if (response.ok) {
                ...
                try {
                    const lsBaseKey = 'preview_base_customize_' + websiteId;
                    const stored = localStorage.getItem(lsBaseKey);
    ...
    function restoreStoredPreviewState() {
        try {
            ...
            if (storedSection) {
                ...
                const tab = document.querySelector('.settings-nav-tab[data-section="' + storedSection + '"]');
                if (tab) {
                    tab.click();
                }
            }
...
<?php /**PATH C:\xampp\htdocs\A.R.T.C\resources\views/smartprep/dashboard/partials/customize-scripts.blade.php ENDPATH**/ ?><script type="text/javascript">
    // Settings tab navigation with enhanced functionality
    document.addEventListener('DOMContentLoaded', function() {
        const navTabs = document.querySelectorAll('.settings-nav-tab');
        const sidebarSections = document.querySelectorAll('.sidebar-section');
