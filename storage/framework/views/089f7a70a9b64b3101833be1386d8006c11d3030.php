                    ...
                    <form id="generalForm" method="POST" action="<?php echo e(route('smartprep.admin.settings.update.general')); ?>">
                        ...
                        <?php
                            ...
                            if (empty($derivedAdminEmail)) {
                                // If a client slug was provided via query (?client_slug=), try to use it
                                $clientSlug = request()->query('client_slug');
                                $slugSource = $clientSlug ?: ($settings['general']['brand_name'] ?? 'artc');
                                $slugSource = preg_replace('/^smartprep-/', '', $slugSource);
                                $domainBase = strtolower(preg_replace('/[^a-z0-9]+/', '', $slugSource));
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const configured = (settings.data && settings.data.general && settings.data.general.preview_url)
                        ...
                        : fallbackUrl;
                    let baseToUse = storedBase || configured;
                    if (!baseToUse) baseToUse = fallbackUrl;
                    // If section implies panel path, adjust base
                    if (storedSection) {
                        switch(storedSection) {
                            case 'student':
                                if (!/\/student\/dashboard$/.test(baseToUse)) baseToUse = baseToUse.replace(/\/$/, '') + '/student/dashboard';
                                break;
                            case 'professor':
                                if (!/\/professor\/dashboard$/.test(baseToUse)) baseToUse = baseToUse.replace(/\/$/, '') + '/professor/dashboard';
                                break;
                            case 'admin':
                                if (!/\/admin-dashboard$/.test(baseToUse)) baseToUse = baseToUse.replace(/\/$/, '') + '/admin-dashboard';
                                break;
                            case 'homepage':
                            case 'branding':
                            case 'navbar':
                                // use root
                                baseToUse = configured;
                                break;
                        }
                    }
                    const finalUrl = baseToUse + (baseToUse.includes('?') ? '&' : '?') + 'preview=true&t=' + Date.now();
                    iframe.src = finalUrl;
                } else {
                    console.warn('Settings fetch failed with status', response.status);
                }
            } catch (e) {
