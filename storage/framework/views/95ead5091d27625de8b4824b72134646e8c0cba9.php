    ...
    <link rel="stylesheet" href="<?php echo e(asset('css/ENROLLMENT/Full_Enrollment.css')); ?>">
    <script src="<?php echo e(asset('js/enrollment/full_enrollment.js')); ?>"></script>
    ...
    const isUserLoggedIn = <?php echo e($userLoggedIn ? 'true' : 'false'); ?>;
    const loggedInUserId = '<?php echo e($userId); ?>';
    const loggedInUserName = '<?php echo e($userName); ?>';
    const loggedInUserFirstname = '<?php echo e($userFirstname); ?>';
    const loggedInUserLastname = '<?php echo e($userLastname); ?>';
    const loggedInUserEmail = '<?php echo e($userEmail); ?>';
    ...
    const hasRecaptcha = <?php if(env('RECAPTCHA_SITE_KEY')): ?> true <?php else: ?> false <?php endif; ?>;

    // Plan configuration data
    const planConfig = {
        full: {
            enableSynchronous: <?php echo e(isset($fullPlan) && $fullPlan->enable_synchronous ? 'true' : 'false'); ?>,
            enableAsynchronous: <?php echo isset($fullPlan) && $fullPlan->enable_asynchronous ? 'true' : 'false'; ?>

        },
        modular: {
            enableSynchronous: <?php echo isset($modularPlan) && $modularPlan->enable_synchronous ? 'true' : 'false'; ?>,
            enableAsynchronous: <?php echo isset($modularPlan) && $modularPlan->enable_asynchronous ? 'true' : 'false'; ?>
    ...
    <div class="form-container">
        <div class="form-wrapper">
            ...
            <div class="stepper-progress">
                <div class="stepper">
                    <div class="bar">
                        <div class="progress" id="progressBar" style="width: <?php echo e($userLoggedIn ? '25%' : '20%'); ?>;"></div>
                    </div>
                    <div class="step <?php echo e(!$userLoggedIn ? 'active' : ''); ?>" id="step-1">
                        <div class="circle">1</div>
                        <div class="label">Account Check</div>
                    </div>
                    <div class="step <?php echo e($userLoggedIn ? 'active' : ''); ?>" id="step-2">
            ...
            <form action="<?php echo e(route('student.register')); ?>" method="POST" enctype="multipart/form-data" class="registration-form" id="enrollmentForm" novalidate onsubmit="return handleFormSubmission(event)">
                <?php echo csrf_field(); ?>
                ...
                <div class="step-content <?php echo e(!$userLoggedIn ? 'active' : ''); ?>" id="step-content-1">
                ...
                <div class="step-content <?php echo e($userLoggedIn ? 'active' : ''); ?>" id="step-content-2">
                    <div class="step-header mb-4">
                                    ...
                                    <div class="d-flex justify-content-center gap-4 flex-wrap">
                                        <?php $__currentLoopData = $packageChunk; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $package): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <div class="package-card-pro card p-4 mb-3"
                                                 onclick="selectPackage('<?php echo e($package->package_id); ?>', '<?php echo e(addslashes($package->package_name)); ?>', '<?php echo e($package->amount); ?>')"
        ...
        <div class="step-content" id="step-content-3">
            ...
            <div class="row justify-content-center">
                <?php if(isset($fullPlan) && $fullPlan->enable_synchronous): ?>
                ...
                <?php if(isset($fullPlan) && $fullPlan->enable_asynchronous): ?>
                <div class="col-md-5 mb-4">
    ...
    <div class="step-content" id="<?php echo e($userLoggedIn ? 'step-content-4' : 'step-content-5'); ?>">
        <div class="student-step-card">
            ...
            <?php if(isset($formRequirements) && $formRequirements->count() > 0): ?>
                ...
                <?php $__currentLoopData = $formRequirements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    <?php else: ?>
                        <?php if($field->field_name !== 'Cert_of_Grad' && $field->field_name !== 'firstname' && $field->field_name !== 'lastname'): ?>
                            <div class="form-group">
                                ...
                                <?php elseif($field->field_type === 'file'): ?>
                                    <div class="form-group" id="file-group-<?php echo e($field->field_name); ?>">
                                        <label for="<?php echo e($field->field_name); ?>"><?php echo e($field->field_label); ?></label>
                                        <input type="file" name="<?php echo e($field->field_name); ?>" id="<?php echo e($field->field_name); ?>" onchange="handleFileUpload(this)">
                                        <div class="uploaded-file-info" id="uploaded-<?php echo e($field->field_name); ?>" style="display:none;"></div>
                                    </div>
                    ...
                    <?php if(isset($educationLevels) && $educationLevels->count() > 0): ?>
                        <?php $__currentLoopData = $educationLevels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $level): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($level->level_name); ?>" 
                                    data-file-requirements="<?php echo e(json_encode($level->getFileRequirementsForPlan($enrollmentType ?? 'full'))); ?>">
            ...
            <div class="form-group" style="margin-top:2rem;">
                ...
                <input type="date" name="Start_Date" id="start_date_input" class="form-control"
                    value="<?php echo e($student->start_date ?? old('Start_Date')); ?>" 
...
<?php echo nl2br(e(\App\Models\AdminSetting::getValue('full_enrollment_terms', 'By registering for this platform, you agree to abide by all policies, privacy guidelines, and usage restrictions as provided by our review center. Please read the full document before accepting.'))); ?>
...
if (currentStep !== finalStep) {
  ...
  return false;
    ...
    async function sendEnrollmentOTP() {
        ...
        try {
            const response = await fetch('<?php echo e(route("enrollment.send-otp")); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    async function verifyEnrollmentOTP() {
        ...
        try {
            const response = await fetch('<?php echo e(route("enrollment.verify-otp")); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': CSRF_TOKEN
    ...
    async function checkEmailAvailability(email) {
        ...
        try {
            const response = await fetch('<?php echo e(route("check.email.availability")); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': CSRF_TOKEN
...
<?php echo $__env->make('layouts.navbar', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\A.R.T.C\resources\views/registration/Full_enrollment.blade.php ENDPATH**/ ?>