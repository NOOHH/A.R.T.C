...
<head>
    ...
    <style>
        ...
        .settings-nav-tab.active .portal-preview-link {
            color: rgba(255, 255, 255, 0.8);
        }
        
        .settings-nav-tab.active .portal-preview-link:hover {
            color: white;
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="<?php echo e(route('dashboard.customize-website')); ?>">
            ...
            <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="settings-nav-tabs">
                        ...
                        <button class="settings-nav-tab" data-section="homepage" style="position: relative;">
                            <i class="fas fa-home me-2"></i>Homepage
                            <a href="#" onclick="navigatePreview('/preview/artc-template'); event.stopPropagation(); return false;" class="portal-preview-link" title="Preview Homepage">
                                <i class="fas fa-external-link-alt ms-2"></i>
                            </a>
                        </button>
                        <button class="settings-nav-tab" data-section="student" style="position: relative;">
                            <i class="fas fa-user-graduate me-2"></i>Student Portal Settings
                            <a href="#" onclick="navigatePreview('/preview/student-portal'); event.stopPropagation(); return false;" class="portal-preview-link" title="Preview Student Portal">
                                <i class="fas fa-external-link-alt ms-2"></i>
                            </a>
                        </button>
                        <button class="settings-nav-tab" data-section="professor" style="position: relative;">
                            <i class="fas fa-chalkboard-teacher me-2"></i>Professor Panel Settings
                            <a href="#" onclick="navigatePreview('/preview/professor-panel'); event.stopPropagation(); return false;" class="portal-preview-link" title="Preview Professor Panel">
                                <i class="fas fa-external-link-alt ms-2"></i>
                            </a>
                        </button>
                        <button class="settings-nav-tab" data-section="admin" style="position: relative;">
                            <i class="fas fa-user-shield me-2"></i>Admin Panel Settings
                            <a href="#" onclick="navigatePreview('/preview/admin-panel'); event.stopPropagation(); return false;" class="portal-preview-link" title="Preview Admin Panel">
                                <i class="fas fa-external-link-alt ms-2"></i>
                            </a>
                        </button>
...
<meta property="og:type" content="website"></textarea>
                ...
                <div class="preview-header">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <div class="preview-controls">
                            ...
                            <button class="preview-btn btn-success" onclick="applyCustomizations()" id="applyCustomizationsBtn">
                                <i class="fas fa-magic me-1"></i>Apply to Website
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            navTabs.forEach(tab => {
                tab.addEventListener('click', function() {
                    ...
                    // Auto-navigate preview to matching portal
                    autoNavigatePreview(section);
                });
            });
        ...
        // Auto-navigate preview based on settings section
        function autoNavigatePreview(section) {
            const previewUrls = {
                'general': '/preview/artc-template',
                'branding': '/preview/artc-template',
                'navbar': '/preview/artc-template',
                'homepage': '/preview/artc-template',
                'student': '/preview/student-portal',
                'professor': '/preview/professor-panel',
                'admin': '/preview/admin-panel',
                'advanced': '/preview/artc-template'
            };
            
            if (previewUrls[section]) {
                navigatePreview(previewUrls[section]);
            }
        }
        ...
        async function handleFormSubmission(event, settingType, loadingText) {
            ...
            try {
                ...
                const response = await fetch(`/dashboard/customize-website/${settingType}`, {
                    method: 'POST',
                    headers: {
        ...
        // Navigate preview iframe to specific portal
        function navigatePreview(url) {
            ...
            if (iframe && loading) {
                ...
                // Update the iframe onload to hide loading
        ...
        function navigateToHome() {
            navigatePreview('/preview/artc-template');
        }

        // Apply customizations to existing website
        function applyCustomizations() {
            const activeWebsites = <?php echo json_encode($activeWebsites, 15, 512) ?>;
            
            if (activeWebsites.length === 0) {
                alert('You need to have an active website to apply customizations. Please submit a website request first.');
                return;
            }
            
            if (activeWebsites.length === 1) {
                // Only one website, apply directly
                applyCustomizationsToWebsite(activeWebsites[0].slug);
            } else {
                // Multiple websites, show selection modal
                showWebsiteSelectionModal(activeWebsites);
            }
        }

        function applyCustomizationsToWebsite(clientSlug) {
            const btn = document.getElementById('applyCustomizationsBtn');
            const originalText = btn.innerHTML;
            
            btn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Applying...';
            btn.disabled = true;
            
            fetch('<?php echo e(route("dashboard.apply-customizations")); ?>', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    showSuccessMessage('Customizations applied successfully! Your website has been updated.');
                    // Refresh preview to show changes
                    setTimeout(() => {
                        refreshPreview();
                    }, 1000);
                } else {
                    showErrorMessage('Error applying customizations: ' + data.message);
                }
            })
            .catch(error => {
                showErrorMessage('Error applying customizations: ' + error.message);
            })
            .finally(() => {
        ...
        function showWebsiteSelectionModal(websites) {
            const modal = document.createElement('div');
            modal.className = 'modal fade';
            modal.id = 'websiteSelectionModal';
            modal.innerHTML = `
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">
                                <i class="fas fa-globe me-2"></i>Select Website
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <p>You have multiple websites. Which one would you like to apply customizations to?</p>
                            <div class="list-group">
                                ${websites.map(website => `
                                    <button type="button" class="list-group-item list-group-item-action" 
                                            onclick="applyCustomizationsToWebsite('${website.slug}'); $('#websiteSelectionModal').modal('hide');">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div>
                                                <strong>${website.name}</strong>
                                                <br><small class="text-muted">${website.slug}</small>
                                            </div>
                ...
                <form method="POST" action="<?php echo e(route('dashboard.submit-customized-website')); ?>" id="customizedWebsiteForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label">Contact Email *</label>
                                <input type="email" class="form-control form-control-modern" name="contact_email" required value="<?php echo e(Auth::user()->email); ?>">
                            </div>
                            <div class="col-md-6">
...
<?php /**PATH C:\xampp\htdocs\A.R.T.C\resources\views\dashboard\customize-website.blade.php ENDPATH**/ ?>